...
/*/
//--------------------------------------------------------------------
User Function UPD0226( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD0226" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD0226" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD0221" ) ) ) ;
...
If !AcheiSX3("SRG","RG_XRESDIS")
   cOrd1 := UltimoSX3("SRG") 
   cOrd1 := Soma1(cOrd1) 
Else
   cOrd1 := QualOrdemSX3("SRG","RG_XRESDIS")
Endif

aAdd( aSX3, { ;
	{'SRG'																	, .T. }, ; //X3_ARQUIVO
	{ cOrd1											                        , .T. }, ; //X3_ORDEM
	{'RG_XRESDIS'															, .T. }, ; //X3_CAMPO
	{'C'																	, .T. }, ; //X3_TIPO
	{1																		, .T. }, ; //X3_TAMANHO
	{0																		, .T. }, ; //X3_DECIMAL
	{'TipoResc'																, .T. }, ; //X3_TITULO
	{'TipoResc'																, .T. }, ; //X3_TITSPA
	{'TipoResc'																, .T. }, ; //X3_TITENG
	{'Tipo Resc Rededor'													, .T. }, ; //X3_DESCRIC
	{'Tipo Resc Rededor'													, .T. }, ; //X3_DESCSPA
	{'Tipo Resc Rededor'													, .T. }, ; //X3_DESCENG
	{'@!'																	, .T. }, ; //X3_PICTURE
	{''																		, .T. }, ; //X3_VALID
	{Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(160)					, .T. }, ; //X3_USADO
	{'IF(CCOMPL="N","0"," ")'												, .T. }, ; //X3_RELACAO
	{''																		, .T. }, ; //X3_F3
	{1																		, .T. }, ; //X3_NIVEL
	...
	{'€'																	, .T. }, ; //X3_OBRIGAT
	{''																		, .T. }, ; //X3_VLDUSER
	{'0=Original;1=Complementar;2=ComplementarDissidio;3=Retificacao'		, .T. }, ; //X3_CBOX
	{'0=Original;1=Complementar;2=Complementar Dissidio;3=Retificacao'		, .T. }, ; //X3_CBOXSPA
	{'0=Original;1=Complementar;2=Complementar Dissidio;3=Retificacao'		, .T. }, ; //X3_CBOXENG
	{''																		, .T. }, ; //X3_PICTVAR
	{''																		, .T. }, ; //X3_WHEN
	{''																		, .T. }, ; //X3_INIBRW
	{''																		, .T. }, ; //X3_GRPSXG
...
///////////////////////////////////////////////////////////
If !AcheiSX3("SRG","RG_XRESDIS")
   aAdd( aHlpPor, 'Informe o indicador de Tipo de       ' )
   aAdd( aHlpPor, 'Rescisao    :' )
   aAdd( aHlpPor, '0-Original         ;' )
   aAdd( aHlpPor, '1-Complementar;' )
   aAdd( aHlpPor, '2-Complementar Dissidio;' )
   aAdd( aHlpPor, '3-Retificacao.' )
   aAdd( aHlpPor, 'O campo é de preenchimento obrigatório' )
   //////PutHelp( "PRG_XRESDIS", aHlpPor, {}, {}, .T. )
   AutoGrLog( "Atualizado o Help do campo " + "RG_XRESDIS" )
Endif
///////////////////////////////////////////////////////////

AutoGrLog( "Fim da Atualização" + " " + "Helps de Campos" + CRLF )

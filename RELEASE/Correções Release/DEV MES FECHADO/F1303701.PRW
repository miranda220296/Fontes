...
User Function F1303701(aCabNota, cData, cNovIdInt, nRecLog, cEmpLog, cFilLog, cDocNum)
	...
	Local nSF1Recno     := 0
	Local nSM0Recno     := 0
    Local aTables       := {"P50","SF1","SD1","SD2","SF2"}

	Private cErroL          := ""
	Private lAutoErrNoFile  := .T.
    Private nRec      := nRecLog
	Default aCabNota    := {}
	Default cData       := ""
	Default nRecLog     := 0
	...
	If !(Select("SX2") > 0)
		...
		Else
            ...
            RpcSetEnv( cEmpLog,cFilLog,,,,, aTables, , , ,  )
			If !(Select("SX2") > 0)
    ...
    U_MSINTNFS("A",{},nRecLog,{Iif(Empty(cResposta), "OK| Processo efetuado com sucesso.",cResposta), Iif(lsucesso, 1, 2)},2)
...
User Function F1303707(oCabDocEnt, cData, cNovIdInt, nRecLog, cRetorno, nOperacao)
	...
	If nOperacao == 1
		...
		nRecLog := U_MSINTNFS("I",{oCabDocEnt},0,{},4)//Novo LOG da integração de NFS
	Else
		nRecLog := U_MSINTNFS("I",{oCabDocEnt},0,{},6)//Novo LOG da integração de NFS
    ...
    If !lValidado
        ...
        U_MSINTNFS("A",{},nRecLog,{cRetorno,2},2)
    EndIf
    ...
    If !(Empty(cIdInteg)) .And. nOperac <> 0
        If nOperac == 3
            ...
            If SC7->(DbSeek(cIdInteg))
                cNumPed := SC7->C7_NUM
                U_F0702203(cNumPed, nOperac, 1)//U_F0702203( cPedComp, nOper, nOpca, nRecLogSC7)
            EndIf
        ElseIf nOperac == 5
            SC7->(DbOrderNickname("EF1304401"))
            SET DELETED OFF
            If SC7->(DbSeek(cIdInteg))
                nRecLognoSC7   := SC7->(Recno())
                cNumPed     := SC7->C7_NUM
            EndIf
            SET DELETED ON

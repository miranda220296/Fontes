...
//				If n > 1 .And. cMyCod == GDFieldGet ( "C7_PRODUTO" ,N,.F.,,)
			...
			If !lContrat .And. !Isblind()
				aRet := U_MSCHKCOND(CCONDICAO,CA120FORN,CA120LOJ)

				If aRet[3]
					If MsgYesNo("Há múltiplas parcelas programadas para o mesmo dia. Este processo possui adiantamento?")
						lAdiant := .T.
					EndIf
				EndIf
				...
				If !lAdiant
					If !aRet[1]
						Help( , , 'Help', 'MSCHKCOND', aRet[2], 1, 0, , , , , , {"Revise a condição de pagamento escolhida."})
						lRet := aRet[1]
					EndIf
				Else
					For nY := 01 To Len(aCols)
						aCols[nY][GdFieldPos("C7_XADIANT")] := "S"
					Next nY
				EndIf
			EndIf
		Else

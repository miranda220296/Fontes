...
User Function AEMailFin(_nRecNo,_cTipo)
	...
	Local lAutentica	:= .T.
	Local aEmailConfig	:= {} 
	Local nSMTPPort 	:= GetMV("MV_GCPPORT",, 25)
	...
	For ni := 1 to Len(aEmailParam)
		aAdd(aEmailConfig,SuperretMV(aEmailParam[ni][1],.F.,Nil))
	...
	nErr := oServer:init("",_MailS, aEmailConfig[2],aEmailConfig[3],,nSMTPPort)
	If nErr <> 0	
...
// Objeto de Email
	...
	// Dados da mensagem		
	oMessage:cFrom		:= aEmailConfig[4]  
	oMessage:cTo     	:=  _cMlCola
	oMessage:cCC    	:=  _cMlSoli
	oMessage:cSubject	:= _cSubject
	oMessage:cBody   	:= _cMsg
...
Return
static function SuperretMV(aVar, lVar, cVar2)


	return SuperGetMV(aVar, lVar, cVar2)

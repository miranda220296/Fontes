...
SetPrvt( "DDATAREF, DDATALIB, ESC, SEMANA, cFilDe, CFILATE" )
...
//---------->Utilizando as Perguntas do Santander
Private cPerg	 := "FSW0201301"
Private cSem_De  := "  /  /    "
Private cSem_Ate := "  /  /    "
...
*/
//=---------------->adequação ao padrão do Santander
While At("'",mv_par05)
	mv_par05 := STUFF(mv_par05,At("'",mv_par05),1,"")
endDo
...
cProcesso  := Alltrim(mv_par05)	//Processo
cRoteiro   := mv_par01 	//Emitir Recibos(Roteiro)
If cRoteiro $ '131|132' //cRoteiro == '132' //10029928 - Thais Paiva
	cPeriodo   := Substr(DtoS(MV_PAR11),1,4)+SubStr(DtoS(MV_PAR11),5,2) //10029928 - Thais Paiva
Else //10029928 - Thais Paiva
	// Início -- ticket 10568741
	If Month(MV_PAR11) <> 1
		cPeriodo := Substr(DtoS(MV_PAR11),1,4)+StrZero(Val(SubStr(DtoS(MV_PAR11),5,2))-1,2)
	Else
		cPeriodo := cValToChar(Year(MV_PAR11) - 1) + "12"
	EndIf
	// Fim -- ticket 10568741  
EndIf //10029928 - Thais Paiva
Semana     := mv_par06 	//Numero da Semana
...
//cFilDe     := mv_par05
//cFilAte    := mv_par06
//-------->Customização solicitada por Rede D'or - 06/12/19
cFilDe     := mv_par02
cFilAte    := mv_par02
cCcDe      := IiF(!Empty(mv_par03),IIf(At("-",mv_par03)>0,SubStr(mv_par03,1,At("-",mv_par03)-1),mv_par03),mv_par03) //-- ajustar leitura da variavel
cCcAte     := Alltrim(Iif(!Empty(mv_par03),IIf(At("-",mv_par03)>0,SubStr(mv_par03,At("-",mv_par03)+1,Len(mv_par03)),mv_par03),mv_par03))  //-- ajustar leitura da variavel
cMatDe     := IiF(!Empty(mv_par04),IIf(At("-",mv_par04)>0,SubStr(mv_par04,1,At("-",mv_par04)-1),mv_par04),mv_par04) //-- ajustar leitura da variavel
cMatAte    := Alltrim(Iif(!Empty(mv_par04),IIf(At("-",mv_par04)>0,SubStr(mv_par04,At("-",mv_par04)+1,Len(mv_par04)),mv_par04),mv_par04))  //-- ajustar leitura da variavel
cNomDe     := IiF(!Empty(mv_par07),IIf(At("-",mv_par07)>0,SubStr(mv_par07,1,At("-",mv_par07)-1),mv_par07),mv_par07)  //-- ajustar leitura da variavel
cNomAte    := Alltrim(Iif(!Empty(mv_par07),IIf(At("-",mv_par07)>0,SubStr(mv_par07,At("-",mv_par07)+1,Len(mv_par07)),mv_par07),mv_par04))  //-- ajustar leitura da variavel
cSituacao  := mv_par08
cCategoria := mv_par09
lTipoRem   := 2 //-- TIPO DE REMESSA, TESTE = 1 OU PRODUÇÃO = 2
cNomeArq   := mv_par10
dDatalib   := mv_par11
cNum       := mv_par13
lEhIncBDN  := ( MV_PAR12 == 1 )
...
//Mensag3    := mv_par22
cDeptoDe   := "                          "
cDeptoAte  := "ZZZZZZZZZZZZZZZZZZZZZZZZZZ"
//cCNPJ	   := mv_par25
cCNPJ	   := SM0->M0_CGC
//dDataRef   := mv_par14
...
cTipoRot  :=  PosAlias("SRY",cRoteiro,cFilDe,"RY_TIPO")
dDataPag  :=  PosAlias("RCH",(cProcesso+cPeriodo+Semana+cRoteiro),cFilde,"RCH_DTPAGO")
...
If nHdl > 0
	If fClose( nHdl )
		...
		If nSeq_ <> 0
				If lEhIncBDN
					Aviso( 'AVISO', 'Gerado o arquivo ' + AllTrim( AllTrim( cNomeArq ) ) + CHR(10) + CHR(13)  + ;
...
aAdd( aTipo0, { 'CÓDIGO DA EMPRESA                    ', 022, 030,   'N',   9,  0, 'StrZero(Val( SuperGetMV("MV_XRCBRHL",.T.,"123456789",cFilde) ),9)' } )
aAdd( aTipo0, { 'NÚMERO DO LOTE                       ', 031, 039,   'N',   9,  0, 'StrZero(Val(cNum),9)' } )
...
aAdd( aTipo0, { 'REFERÊNCIA DA OPERAÇÃO DO LOTE       ', 063, 063,   'C',   1,  0, 'IIF( MV_PAR12==1, "I", "S" )' } )
...
aAdd( aTipo1, { 'DATA LIBERAÇÃO COMPROVANTE(DDMMAAAA) ', 012, 019,   'N',   8,  0, 'GravaData( DataValida( dDataLib ), .F., 5 )' } )
...
aAdd( aTipo4, { 'DATA INICIO PERIODO AQUISITIVO       ', 031, 038,   'C',   8,  0, 'cDtInPer' } )
aAdd( aTipo4, { 'DATA FINAL  PERIODO AQUISITIVO       ', 039, 046,   'C',   8,  0, 'cDtFnPer' } )
aAdd( aTipo4, { 'DATA INICIO GOZO DE FERIAS           ', 047, 054,   'C',   8,  0, 'cDtInGoz' } )
aAdd( aTipo4, { 'DATA FINAL GOZO DE FERIAS            ', 055, 062,   'C',   8,  0, 'cDtFnGoz' } )
...
//-----------------> Removida opção de filtro por departamento 06/12/19
...
While SRA->(!EOF()) .AND. &cInicio <= cFim
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	//----------------------->Removida opção de departamento
	/*
	...
	*/
	...
	If ! SRH->(dbSeek(cFilde+SRA->RA_MAT+DTOS(dDataRef),.F.))
	...
	Else
		...
		cDtInPer  := SubStr(DTOS(SRH->RH_DATABAS),7,2)+SubStr(DTOS(SRH->RH_DATABAS),5,2)+SubStr(DTOS(SRH->RH_DATABAS),1,4)  
		cDtInGoz  := SubStr(DTOS(SRH->RH_DATAINI),7,2)+SubStr(DTOS(SRH->RH_DATAINI),5,2)+SubStr(DTOS(SRH->RH_DATAINI),1,4)	
		cDtFnPer  := SubStr(DTOS(SRH->RH_DBASEAT),7,2)+SubStr(DTOS(SRH->RH_DBASEAT),5,2)+SubStr(DTOS(SRH->RH_DBASEAT),1,4)
		cDtFnGoz  := SubStr(DTOS(SRH->RH_DATAFIM),7,2)+SubStr(DTOS(SRH->RH_DATAFIM),5,2)+SubStr(DTOS(SRH->RH_DATAFIM),1,4)
...
/*Compatibilização Release 27 - Thais Paiva
If cNum == 'NAOEXISTE'
	RecLock("SX6",.t.)
...
For nI := 1 To 2
	...
	If nI == 2 .AND. Len( aVerbas ) == 0
		aAdd(aVerbas,{"000 DEBITO              ",0,0})
	EndIf
...
//--------------->DESATIVADO, UTILIZANDO PERGUNTAS DO RECIBO SANTANDER
/*
...
aAdd( aDados, {cPerg, "19", "Mensagem 1 ?"			 , "¿Mensaje 1?"		 , "Message 1 ?"		 , "mv_chj", "C", 1						, 0, 0, "G", ""															, "mv_par19", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "S036"	, "", ""	, ""} )
aAdd( aDados, {cPerg, "20", "Mensagem 2 ?"			 , "¿Mensaje 2?"		 , "Message 2 ?"		 , "mv_chk", "C", 1						, 0, 0, "G", ""															, "mv_par20", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "S036"	, "", ""	, ""} )
aAdd( aDados, {cPerg, "21", "Mensagem 3 ?"			 , "¿Mensaje 3?"		 , "Message 3 ?"		 , "mv_chl", "C", 1						, 0, 0, "G", ""															, "mv_par21", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "S036"	, "", ""	, ""} )
aAdd( aDados, {cPerg, "22", "Depto De ?"			 , "Depto De ?"			 , "Depto De ?"			 , "mv_chm", "C", TamSx3("RA_DEPTO")[1]	, 0, 0, "G", ""															, "mv_par22", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SQB"		, "", "025"	, ""} )
aAdd( aDados, {cPerg, "23", "Depto Ate ?"			 , "Depto Ate ?"		 , "Depto Ate ?"		 , "mv_chn", "C", TamSx3("RA_DEPTO")[1]	, 0, 0, "G", "naovazio"													, "mv_par23", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SQB"		, "", "025"	, ""} )
aAdd( aDados, {cPerg, "24", "Data de Liberação ?"	 , "Data de Liberação?"	 , "Data de Liberação ?" , "mv_cho", "D", 8						, 0, 0, "G", "naovazio"													, "mv_par24", ""		, ""		, ""		, "", "", ""			, ""			, ""			, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SQB"		, "", ""	, ""} )

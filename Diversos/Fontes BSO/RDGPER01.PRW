...
/*/{Protheus.doc} RDGPER01
    ...
    @Thiago Pereira
/*/
User Function  RDGPER01    
Local aArea   	:= GetArea()
Local cTitulo   := 'Relatório Compara SRD x RHP'
Private cPerg   := ""
Private cFils  := "" 
Private cVerbas := "" 
Private cMats  := "" 
...
Local nPosIni	:=	1
Local i := 0
Local _cPath := "C:\DIVER_SRD_RHP\" 
Local _cArq := "C:\DIVER_SRD_RHP\DIVER_SRD_RHP_"+DtoS(DATE())+".CSV"
Local _lArqCsv 
Local _cLin  
Local _cScript := "C:\DIVER_SRD_RHP\DIVER_SRD_RHP"
Local _cEscrev
Local cxQry := "" 
Local _nContar := 0 
Local _lGerouExc := .F.           
Local _aCamposAlias := {}
	        ...
	        aAdd(_aCamposAlias, { "DT_COM PETENCIA", TamSX3("RHP_COMPPG")[3], TamSX3("RHP_COMPPG")[1], TamSX3("RHP_COMPPG")[2]})
            aAdd(_aCamposAlias, { "RD_MAT", TamSX3("RD_MAT")[3],  TamSX3("RD_MAT")[1], TamSX3("RD_MAT")[2]}) 
  			...
  			aAdd(_aCamposAlias, { "RHP_VLRFUN" ,  TamSX3("RHP_VLRFUN")[3], TamSX3("RHP_VLRFUN")[1], TamSX3("RHP_VLRFUN")[2]})
...
cxQry += " NVL(RHP_VLRFUN,0.00) VALOR_PLANO,				" +ENTER				
cxQry += " (NVL(SUM(RD_VALOR),0.00) - NVL(RHP_VLRFUN,0.00)) DIFERENCA	" +ENTER						
...
cxQry += "           WHERE RHP.D_E_L_E_T_    <> '*' 			" +ENTER
	If !Empty(cFils)
		cxQry += "AND RHP_FILIAL IN ("+cFils+") " + ENTER  
	EndIf  
cxQry += " 				AND TRIM(RHP_MAT) >= '"+pMatDe+"' 	    " +ENTER  
cxQry += " 				AND TRIM(RHP_MAT) <= '"+pMatAte+"' 	    " +ENTER  
	If !Empty(cVerbas)
		cxQry += "	AND trim(RHP_PD) IN ("+cVerbas+")   	" 	+ ENTER  
	EndIf 
...
//	   MPSysOpenQuery(cxQry, calias)
	...
	If ! File( "C:\DIVER_SRD_RHP\DIVER_SRD_RHP.BAT" )
		_lArqCsv := FCreate( "C:\DIVER_SRD_RHP\DIVER_SRD_RHP.BAT", 0 )
		_cLin    := "ECHO OFF" + ENTER
		_cLin    += "del C:\DIVER_SRD_RHP\*LIBERACAO*.csv" + ENTER
		fWrite( _lArqCsv, _cLin, _lArqCsv )
		fClose( _lArqCsv )
	EndIf

	If File( "C:\DIVER_SRD_RHP\LIBERACAO.CSV" )
		Shellexecute( "Open", "C:\DIVER_SRD_RHP\\DIVER_SRD_RHP.BAT", " /k dir ", "c:\", 1 )
	EndIf
 	...
 	If Select(cAliasT) >0 
		...
		_lArqCsv := FCreate( _cArq, 0 )
 		...
 		_cEscrev := 'DIVERGENCIA ENTRE SRD X RHP' + ENTER
	...
	If _lGerouExc
		...
		Aviso( 'INFO', 'O Relatorio de DIVERGêNCIA ENTRE SRD X RHP está finalizado,' +;

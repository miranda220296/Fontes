...
Static Function AtuSB2(cCodFec, dDtFec, cMsg)
	...
	cUpdate := " UPDATE " + retsqlname("SB2") +  " SET D_E_L_E_T_ = '*', R_E_C_D_E_L_ = R_E_C_N_O_ "
	cUpdate += " WHERE D_E_L_E_T_ = ' ' AND (B2_FILIAL = ' ' OR B2_COD = ' ')"

	If TcSQLExec( cUpdate ) != 0
		Conout("Erro ao tentar atualizar a tabela SB2" + CRLF + TcSQLError())
	Else
		SB2->(dbCommit())
	EndIf

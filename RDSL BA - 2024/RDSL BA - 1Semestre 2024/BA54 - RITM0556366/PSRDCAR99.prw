...
//		if nPosFil == 0 .Or. nPosFor == 0 .Or. nPosLoj == 0 .Or. nPosSer == 0 .Or. nPosDoc == 0 .Or. nPosItem == 0  .Or. nPosTpSp == 0
		if nPosFil == 0 .Or. nPosFor == 0 .Or. nPosLoj == 0 .Or. nPosSer == 0 .Or. nPosDoc == 0 .Or. nPosTpSp == 0
			MsgInfo("Algum campo da chave esta faltando, por favor, verificar. (C7_FILIAL,C7_FORNECE,C7_LOJA,C7_XSERIE,"+CRLF+"C7_XDOC,C7_ITEM,C7_XTPSP)", "Atenção")
			Exit
		endif
		...
		aSort(aDados, , , {|x, y| x[nPosFil]+x[nPosTpSp]+x[nPosFor]+x[nPosLoj]+x[nPosSer]+x[nPosDoc] < y[nPosFil]+y[nPosTpSp]+y[nPosFor]+y[nPosLoj]+y[nPosSer]+y[nPosDoc]})
					...
					if lExist
						...
						If nY == 1
							If Empty(cDadosBkp)
								nItem := 1
							Else
								If AllTrim(cDadosBkp) == AllTrim(adados[nX][nPosFil]+adados[nX][nPosFor]+adados[nX][nPosloj]+adados[nX][nPosDoc]+adados[nX][nPosSer])
									nItem++
								Else
									nItem := 1
								EndIf
							EndIf
							aAdd(aItAux, {"C7_ITEM", PadL(cValToChar(nItem), TamSX3("C7_ITEM")[1], '0'), Nil})
							cDadosBkp := AllTrim(adados[nX][nPosFil]+adados[nX][nPosFor]+adados[nX][nPosloj]+adados[nX][nPosDoc]+adados[nX][nPosSer])
						EndIf
						If AllTrim(aCabec[nY]) <> "C7_ITEM"
...
Static function fValidDic(aDados)
	...
	DbSelectArea("SC7")
	for nX := 01 To Len(aDados)
		If AllTrim(aDados[nX]) == "C7_XTXEXPE"
			lTemTx := .T.
		EndIf
	...
	If !lTxExp
		cMsg := "Usuário sem acesso para incluir solicitação de pagamento com taxa de expediente, favor revisar o arquivo!"
		lRet := .F.
	EndIf

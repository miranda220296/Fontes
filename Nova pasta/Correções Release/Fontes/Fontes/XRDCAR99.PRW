...
User Function XRDCAR99()
	...
	if !lFilSimp 
		Alert("Não é possivel executar o upload de SP do Full acessando pela filial do Simplififado.")
		Return
	endif
		...
		*/
		...
		For nX := 01 To len(aDados)
			...
			If cChave == cChaveAux
				...
				aAdd(aIt, aItAux)
			Else
				aAdd(aError, U_XRDCAR98(aCab, aIt, 3, .F.))
				aCab := {}
				aIt := {}
				cFilAnt := PadR(aDados[nX][nPosFil], TamSx3("C7_FILIAL")[01])
				For nY := 01 To Len(aDados[nX])
...
User Function FRDCAR99(cModel)
	Local cCmpCabec := ""
	Local nX := 0
	oModel2 := FWLoadModel('S0100401')

...
/*/
//--------------------------------------------------------------------
User Function UPD2299B( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD2299" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD2299" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
If !U_AcheiSX3("SRA","RA_XMULVIN")
   cOrd1 := U_UltimoSX3("SRA") 
   cOrd1 := Soma1(cOrd1) 
Else
   cOrd1 := U_QualOrdemSX3("SRA","RA_XMULVIN")
Endif

aAdd( aSX3, { ;
	{ 'SRA'											, .T. }, ; //X3_ARQUIVO
	{ cOrd1											, .T. }, ; //X3_ORDEM
	{ 'RA_XMULVIN'										, .T. }, ; //X3_CAMPO
	{ 'C'											, .T. }, ; //X3_TIPO
	{ 1  											, .T. }, ; //X3_TAMANHO
	{ 0											 , .T. }, ; //X3_DECIMAL
	{ 'M.Vinculos  '										, .T. }, ; //X3_TITULO
	{ 'M.Vinculos  '										, .T. }, ; //X3_TITSPA
	{ 'M.Vinculos  '										, .T. }, ; //X3_TITENG
	{ 'Tem Mult.Vinculos    '									, .T. }, ; //X3_DESCRIC
	{ 'Tem Mult.Vinculos    '									, .T. }, ; //X3_DESCSPA
	{ 'Tem Mult.Vinculos    '     								, .T. }, ; //X3_DESCENG
	{ '@!'											, .T. }, ; //X3_PICTURE
...
aHlpPor := {}
aAdd( aHlpPor, 'Informação Se Funcionario Possui' )
aAdd( aHlpPor, 'Multiplos Vinculos.' )
aHlpEng := {}
aHlpSpa := {}

PutHelp( "PRA_XMULVIN", aHlpPor, aHlpEng, aHlpSpa, .T. )
cTexto += "Atualizado o Help do campo " + "RA_XMULVIN" + CRLF
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD2299" ) ) ) ;

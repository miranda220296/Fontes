...
Static Function MenuDef()  
	...
	AAdd(aRotina,{"+ Detalhes"	, "VIEWDEF.xKPTMntLog"	, 0, 2 } )
	AAdd(aRotina,{"&Alterar"	, "VIEWDEF.xKPTMntLog"  , 0, 4 } )
	AAdd(aRotina,{"&Excluir"	, "VIEWDEF.xKPTMntLog"  , 0, 5 } )

	AAdd(aRotCad,{"00 | Enviar Todos IDs Pendentes"	 		,"U_xKPTOutExc()" 																					 			, 0, 3 } )
	AAdd(aRotCad,{"01 | SM0 - Empresas/Filiais"				,"FWMsgRun(,{|oSay| U_xKPTOSM0(,,,oSay) },'ID: SM001','Enviando Empresas e Filiais...')" 	 			 		, 0, 3 } )
	AAdd(aRotCad,{"02 | SF4 - Tipos de Entrada e Saída"		,"FWMsgRun(,{|oSay| U_xKPTOSF4(,,,oSay) },'ID: SF401','Enviando Cadastro de TES/Tipos de Entrada e Saída...')"	, 0, 3 } )
	AAdd(aRotCad,{"03 | SC7 - Pedidos de Compras"			,"FWMsgRun(,{|oSay| U_xKPTOSC7(,,,,,oSay) },'ID: SC701','Enviando Pedidos de Compras...')" 	 		 			, 0, 3 } )
	AAdd(aRotCad,{"04 | SB1 - Produtos"						,"FWMsgRun(,{|oSay| U_xKPTOSB1(,,,oSay) },'ID: SB101','Enviando Produtos...')" 	 					 			, 0, 3 } )
	AAdd(aRotCad,{"05 | SED - Natureza"						,"FWMsgRun(,{|oSay| U_xKPTOSED(,,,oSay) },'ID: SED01','Enviando Naturezas...')" 	 					 		, 0, 3 } )

	AAdd(aRotCad,{"06 | SE4 - Condição de Pagamentos"		,"FWMsgRun(,{|oSay| U_xKPTOSE4(,,,oSay) },'ID: SE401','Enviando Condição de Pagamentos...')" 	 				, 0, 3 } )
	AAdd(aRotCad,{"07 | CTT - Centro de Custo"				,"FWMsgRun(,{|oSay| U_xKPTOCTT(,,,oSay) },'ID: CTT01','Enviando Centro de Custo...')" 	 						, 0, 3 } )
	AAdd(aRotCad,{"08 | SD1 - Historico TES"				,"FWMsgRun(,{|oSay| U_xKPTOSD1(,,,oSay) },'ID: SD101','Enviando Historico de TES...')" 	 						, 0, 3 } )
	AAdd(aRotCad,{"09 | P02 - Tipo de Requisição"	   		,"FWMsgRun(,{|oSay| U_xKPTOP02(,,,oSay) },'ID: P0201','Enviando Cadastro de Tipo de Requisição...')"			, 0, 3 } )
	AAdd(aRotCad,{"10 | P11 - Setores"	    				,"FWMsgRun(,{|oSay| U_xKPTOP11(,,,oSay) },'ID: P1101','Enviando Cadastro de Setores...')" 	 					, 0, 3 } )
	AAdd(aRotCad,{"11 | P13 - Fabricantes"	    			,"FWMsgRun(,{|oSay| U_xKPTOP13(,,,oSay) },'ID: P1301','Enviando Cadastro de Fabricantes...')" 	 				, 0, 3 } )
	AAdd(aRotCad,{"12 | KT0 - Perfil de Acesso"    			,"FWMsgRun(,{|oSay| U_xKPTOKT0(,,,oSay) },'ID: KT001','Enviando Perfil de Acesso...')" 	 						, 0, 3 } )
	AAdd(aRotCad,{"13 | SC7 - Pedidos de Compras Manual"	,"FWMsgRun(,{|oSay| U_xKPTOSC7(,,,,oSay,,.T.) },'ID: SC701','Enviando Pedido de Compras...')" 	 				, 0, 3 } )
	...
	aAdd( aRotina, {"01 | Outbound", aRotCad        , 0, 4 } ) 
	// ADD OPTION aRotina 	TITLE "01 | Outbound"	 			 		ACTION aRotCad 	OPERATION MODEL_OPERATION_UPDATE ACCESS 2

	aAdd( aRotImp, {"00 | Importar IDs Pendentes", "U_xKPTInbound()"	, 0, 3} ) 
	//	ADD OPTION aRotImp TITLE "00 | Importar IDs Pendentes"		    ACTION "U_xKPTInbound()" OPERATION MODEL_OPERATION_INSERT ACCESS 2	


	AAdd(aRotina,{"02 | Inbound"	,aRotImp, 0, 4 } ) 
	// 	ADD OPTION aRotina TITLE "02 | Inbound"	ACTION aRotImp	OPERATION MODEL_OPERATION_UPDATE ACCESS 2

	AAdd(aRotExe,{"00 | Todos IDs Pendentes"			  	, "U_xKPTInbExc()" 				 		, 0, 3 } )	
	AAdd(aRotExe,{"01 | Processar Posicionado"		    	, "U_xKPTInbExc(,.T.)" 			 		, 0, 3 } )		
	AAdd(aRotExe,{"02 | SF101 - Pré Nota"					, "U_xKPTInbExc(,.F.,'"+cPreNota+"')" 	, 0, 3 } )	
	AAdd(aRotExe,{"03 | SF102 - Doc. Entrada"				, "U_xKPTInbExc(,.F.,'"+cDocEnt+"')" 	, 0, 3 } )	
	AAdd(aRotExe,{"04 | SC704 - Solicitação de Pagamentos"	, "U_xKPTInbExc(,.F.,'"+cSolPag+"')" 	, 0, 4 } )	
...
// 	ADD OPTION aRotExe TITLE "04 | SC704 - Solicitação de Pagamentos"ACTION "U_xKPTInbExc(,.F.,'"+cSolPag+"')"	OPERATION MODEL_OPERATION_UPDATE ACCESS 2
	...
	AAdd(aRotina,{"03 | Executar IDs Protheus"	,aRotExe, 0, 4 } ) 					
	AAdd(aRotina,{"04 | Perfil de Acesso Portal"	, "U_xKPTPerfil()"  , 0, 3 } )

...
/*{Protheus.doc} F1301113
Função chamada no Job final de Substituição
...
User Function F1301113(aParam)
	...
	If !LockByName("F1301113_" + cEmpIni, .F. ,.F.)
		Conout("F1301113 - Ja existe um Job com mesmo nome em execucao!")
	Else
		
		If !RpcSetEnv( cEmpIni, cFilIni,,,"GPE",, /*aFiles*/ )
			Conout("F1301113 - Nao foi possivel inicializar o ambiente.")
		Else	
			dDtAtual := dDatabase
			cQuery := " SELECT PAJ_FILIAL, PAJ_CODIGO, PAJ_FILSUB, PAJ_MATSUB, PAJ_FILFUN, PAJ_MATFUN"
			cQuery += " FROM " + RetSqlName("PAJ") + " PAJ"
			cQuery += " WHERE PAJ_STATUS = '2' "
		    cQuery += " AND PAJ.PAJ_DTFIM <= '" + DTOS(dDtAtual) + "' " 
			...
			While !(cAlias1)->(Eof())
				...
				Begin Transaction
					...
					cQuery2 += " RH3.RH3_XSUBST = 'S' AND"
					cQuery2 += " RH3.RH3_XFILSU = '" + (cAlias1)->(PAJ_FILSUB) + "' AND"
				    cQuery2 += " RH3.RH3_XMATSU = '" + (cAlias1)->(PAJ_MATSUB) + "' " 
					...
					While !(cAlias2)->(Eof())
						...
						If RH3->(DbSeek((cAlias2)->RH3_FILIAL+(cAlias2)->RH3_CODIGO))
							RecLock("RH3",.F.)
							RH3->RH3_FILAPR := (cAlias1)->(PAJ_FILSUB)
							RH3->RH3_MATAPR := (cAlias1)->(PAJ_MATSUB)
							RH3->RH3_XSUBST := ""
							RH3->RH3_XFILSU := ""
							RH3->RH3_XMATSU := ""
							RH3->(MsUnLock())
		...
		UnLockByName("F1301113_" + cEmpIni, .F., .F.)
